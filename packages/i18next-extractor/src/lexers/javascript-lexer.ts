...
import ts, { getLeadingCommentRanges } from 'typescript'


export interface JavascriptLexerOptions extends BaseLexerOptions {
...
export default class JavascriptLexer extends BaseLexer {
	...
	createCommentNodeParser(){
		...
		return (keys: string[], node: ts.Node, content: string) => {
			// console.log({
			// 	test: ts.reduceEachLeadingCommentRange(content, node.getFullStart(), (pos, end, kind, _hasTrailingNewLine, state) => {
